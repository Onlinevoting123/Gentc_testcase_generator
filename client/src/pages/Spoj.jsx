...
const Spoj = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/gemini/e/spoj', { prompt, email })
      ...
      <div className="flex flex-col items-center space-y-8 w-full max-w-md">
        <div className="bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-2xl w-full">
          <form onSubmit={onSubmitHandler} className="space-y-6">
            <img src='https://tse4.mm.bing.net/th/id/OIP.1LN5mWdAye9aaNHQj5gX7wHaHa?pid=ImgDetMain' className="w-10 h-10 object-contain"/>
  ...
  {userData && userData.prompt ? (
    ...
    <p>Given question no. is not present in Spoj</p>
  )}
</div>
